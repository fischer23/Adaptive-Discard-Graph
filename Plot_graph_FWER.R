#Generates Figures 4-6 of the paper "An Adaptive-Discard-Graph for online error control"
...
for(k in 1:n){
  if(d_j[k]>n){
    ...
    w_lda[k,d_j[k]:n]=gamma[(d_j[k]-k):(n-k)]/(1-sum(gamma[1:(d_j[k]-k-1)]))
  }
}
...
p_spending=ggplot(results_df, aes(seq.0.1..0.9..0.1.)) + 
  ...
  geom_line(aes(y = X4, linetype = "50",colour = "2")) + 
  geom_point(aes(y = X4,colour = "2")) +
  geom_line(aes(y = X4.1, linetype = "50",colour = "2"))+
  geom_point(aes(y = X4.1,colour = "2"))+
  geom_hline(yintercept=alpha)+
  scale_colour_manual(guide="none", values=c( "2"="#f84f4f"))+
  scale_linetype_manual(name  ="Batch-size", values = c("1"="solid","10"="longdash","25"="dashed","50"="dotted"))+
...
p_graph=ggplot(results_df_graph, aes(seq.0.1..0.9..0.1.)) + 
  ...
  geom_line(aes(y = X4, linetype = "50",colour = "4")) + 
  geom_point(aes(y = X4,colour = "4", shape = "4")) +
  geom_line(aes(y = X4.1, linetype = "50",colour = "4"))+
  ...
  theme(panel.grid.major = element_line(color = "grey", size = 0.5,linetype = 1),
        panel.border = element_rect(colour = "black", fill=NA, size=1))+
  ggtitle(expression("ADDIS-Graph"[local]))

combined <- p_spending + p_graph  & theme(legend.position = "bottom")
combined + plot_layout(guides = "collect")
ggsave("Plot_FWER_ADDIS_Graph_logq.pdf")
...
combined <- p_spending + p_graph  & theme(legend.position = "bottom")
combined + plot_layout(guides = "collect")
ggsave("Plot_FWER_ADDIS_Graph_q^2.pdf")
...
combined <- p_spending + p_graph  & theme(legend.position = "bottom")
combined + plot_layout(guides = "collect")
ggsave("Plot_FWER_ADDIS_Graph_q^1.6.pdf")
